...
DataManager::DataManager():num_data_(0), socket1_(0), socket2_(0), tot_num_array_data_(0), is_running_(true){
...
void DataManager::SaveDataFromValues(double * data, int st_idx, int data_idx){
    ...
    case QUATERNION:
    {
        Eigen::Quaterniond * tmp_quat = (Eigen::Quaterniond*)data_addr_vec_[data_idx];
        if(data_array_size_[data_idx] != 4){
            printf("[Warning] Array size incorrect: %i th data\n", data_idx);
        }
...
void DataManager::run(){
	...
	printf("[Data Manager] Stop sending data\n");
      }
      //usleep(10000);
      usleep(1000);
    }
